    ...
    */
    ...
    int counter=0;
        while(1)
	    {
	      ...
	      cvInRangeS(p1,cvScalar(0,0,0),cvScalar(179,254, 100),p2);
	      ...
	      if(mode=='m') // @for manual mode...BUT need to revert to automatic also
		    {   
		        //char option = 'm';
		        //transferIn(option); // @ manual mode has been opted... do not actuate just display the mode on the screen
		        // in the board code, set some flag such that 'l' 's' 'r' values are ignored
		        
		        //#$%No need to drive and check for motor actuation...need to only send the type/mode of opperation...But to keep with the transfer of 2 chatacters, send 'n'(none) direction for this mode, and 
		        //#$%Add the 2nd transfer function in common proc and in this field for manual mode
		        
		        
		     	transferIn('m'); // mode
		     	transferIn('n'); // direction - none
		     	
		     	//#$%check the need for the acuate motors parameter, and whether here we need to use the same variables
	      ...
	      {	commonProcessing(cnt, buf, p2, p3, 'a');
	          	actuatemotors = transferOut();	  
        	   	modeLast = 'a';
           	  }
              
          else if(mode==27) // escape = emergency exit/close the application
          	break;
	       
          else if(mode==-1) //redundant case, since mode if = -1 is changed
          { 
	      ...
	      {
	        printf(" No Response by cvKeyWait \n");
	        printf("%d \n", mode);
	       } //replace with actual mode of opperation
	      
              
	    printf("KeyPressed KP KP ::: %c %c %c\n", mode, mode, mode);
...
// multiply and calc max
        ...
        printf("%g, %g, %g; ", result[0], result[1], result[2]);  
